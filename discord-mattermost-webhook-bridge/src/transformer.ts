import { DiscordWebhookPayload, MattermostWebhookPayload, DiscordEmbed, MattermostAttachment } from './types';
...
const formatTimestamp = (text: string): string => {
  return text.replace(/<t:(\d+):(D|F|t|T|d|f|R)>/g, (match, timestamp, format) => {
    ...
    // Format options based on Discord's format codes
    switch (format) {
      case 'D': // Short date: 01/01/2023
        return date.toLocaleDateString();
      case 'F': // Long date with time: January 1, 2023 12:00 PM
        return date.toLocaleString();
      case 't': // Short time: 12:00 PM
        return date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      case 'T': // Long time: 12:00:00 PM
        return date.toLocaleTimeString();
      case 'd': // Long date: January 1, 2023
        return date.toLocaleDateString([], { year: 'numeric', month: 'long', day: 'numeric' });
      case 'f': // Short date with time: January 1, 2023 12:00 PM
        return date.toLocaleString([], { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' });
      case 'R': // Relative time: 2 hours ago
        const now = new Date();
...
const cleanupEmojis = (text: string): string => {
  // Simplified regex to match backticks around text with emojis
  return text.replace(/`([^`]*)`/g, (match, content) => {
    // If content contains emoji, remove the backticks
...
export const transformDiscordToMattermost = (
